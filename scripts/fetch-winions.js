/**
 * Script to fetch WINIONS NFT collection data from Ethereum contract
 * Usage: node scripts/fetch-winions.js
 * 
 * This will fetch all NFTs from the contract and save to src/data/winions.json
 */
...
// Contract address
const CONTRACT_ADDRESS = '0x4ad94fb8b87a1ad3f7d52a406c64b56db3af0733';
const CHAIN = 'ethereum'; // Update if different chain

// Public Ethereum RPC endpoints (no API key needed) - try multiple for reliability
const RPC_URLS = [
...
async function fetchCollectionData() {
  console.log('🔄 Fetching WINIONS NFT collection data...');
  ...
  try {
    ...
    // Connect to Ethereum - try multiple RPC endpoints
    let provider = null;
    let contract = null;
    ...
    const [name, symbol, totalSupply] = await Promise.all([
      contract.name().catch(() => 'WINIONS'),
      contract.symbol().catch(() => 'WINIONS'),
    ...
    // Save to JSON
    const outputPath = path.join(__dirname, '..', 'src', 'data', 'winions.json');
